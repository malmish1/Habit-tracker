(function(){
  ...
  function normalizeHabit(h){const entries={};Object.entries(h.entries||h.checks||{}).forEach(([date,v])=>{if(v===true)entries[date]={status:'done',value:null};else if(v&&typeof v==='object')entries[date]={status:v.status||'done',value:v.value??null}});return{id:h.id||uid(),name:h.name||'Namnlös vana',description:h.description||'',category:h.category||'Övrigt',color:h.color||'#3e7c67',icon:h.icon||'✓',startDate:h.startDate||Object.keys(entries).sort()[0]||new Date().toISOString().slice(0,10),schedule:h.schedule||{type:'daily',weekdays:[1,2,3,4,5,6,0],timesPerWeek:7},target:h.target||null,archived:!!h.archived,createdAt:h.createdAt||new Date().toISOString(),entries};}
